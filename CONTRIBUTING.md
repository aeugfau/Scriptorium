...
| 抽取文本人物建档 | `generators._split_cast`/`_resolve_persons` + `CAST_INSTRUCTION` | LLM 在正文后附 `<CAST>` 块（10列：姓名/性别/身份/阶层/居所/性格/处境/关系类型/标准名/本篇经历）；按「标准名+文明」归并（处理同一人不同称谓），新人物走 `register_commoner`；亲属称谓建双向 `relations` 边；称谓当名由 `naming.AppellationJudge`（LLM 判别+规则兜底）识别→弃用→生成真名；关系类型由 `_normalize_relation` 清洗；本篇经历追加进 `bio_entries` |
