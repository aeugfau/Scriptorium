...
- **判断/提议类 LLM 调用（非文档生成）的健壮性**：这类调用（`NameGenerator`/
  `RoleProposer`/`VoiceReformer`/`BioSummarizer`/`PersonPurger`/`AppellationJudge`）
  必须满足三点：① `max_tokens` 给到 120+（DeepSeek 在 token 预算极小时返回**空串**，
  早期 `AppellationJudge` 因 `max_tokens=10` 致空返回被当成真名，造成「全判真名」假象）；
  ② `try/except` 包住调用，异常时 `raw=""`；③ 空返回走明确兜底（回退规则/mock 补足/
  保守保留），**绝不当成有意义的结果**。文档生成类（体裁生成）不受此约束——其空返回
  由 `_validated` 校验层处理。
...
| 调身份意外风险 | `engine.ACCIDENT_BASE`（基础概率，当前 0.02）+ `ROLE_RISK`（关键词倍率）+ `SOCIAL_CLASS_BASE`（阶层基准）+ `ACCIDENT_CAUSE`（死因映射） | 渔/海最高、贵/祭最低；意外死因按 role 关键词映射（渔→海难）；改 BASE 数字即调整全局意外频率 |
| 加新涌现事件 | `engine._emerge` 追加 if 块 | 涌现应是状态阈值 + 可复现随机；若事件确立不可逆事实，同时写 `Fact`（见下节） |
| 调命名规范 | `models.NamingStyle` + `config.yaml` 各文明 `naming:` 块 | 词库作**风格示例**（LLM 可即兴创造新词，非封闭集）；LLM 造的新词根/氏族由 `NameGenerator._recycle_new_lexemes` 自动回收进词库（每类上限 30）；引擎在科技/政体变更时调 `_maybe_evolve_naming` |
...
| 抽取文本人物建档 | `generators._split_cast`/`_resolve_persons` + `CAST_INSTRUCTION` | LLM 在正文后附 `<CAST>` 块（11列：姓名/性别/身份/阶层/居所/性格/处境/关系类型/标准名/本篇经历/所属组织名）；按「标准名+全文明」归并（处理同一人不同称谓；跨文明文本提及的人物也归并到原卡，不在另一文明重建同名副本），新人物走 `register_commoner`；亲属称谓建双向 `relations` 边；称谓当名由 `naming.AppellationJudge`（LLM 判别+规则兜底）识别→弃用→生成真名；关系类型由 `_normalize_relation` 清洗；本篇经历追加进 `bio_entries`；新人物按 CAST 所属组织（校验存在）或阶层/role/home 推断归入组织 |
| 调社会组织 | `models.OrgType` 枚举 + `models.Organization` + `config.yaml` 各文明 `organizations:` 块 | 核心类型用枚举（确定性逻辑用）；具体组织自由命名；初始组织种子 |
| 加新组织类型 | `models.OrgType` 枚举 | 确定性逻辑靠枚举；具体组织名自由文本涌现 |
| 涌现新组织 | `naming.OrgProposer` | 关键事件（科技/政体/人口）触发；建 Organization 并写 `Fact(kind="org_emergence")`；mock 兜底 `_MOCK_ORG_CANDIDATES` |
...
- 正文为空：DeepSeek 偶发空返回，视为违规触发重生成
- 正文过短：各体裁设最小字符数（编年史 60/日记 50/诏令 40/经文 30/会议纪要 60），低于阈值视为截断触发重生成
- CAST 未闭合：`<CAST>` 开标签无对应 `</CAST>` 时兜底剥离（避免残段污染正文）

`_validated` 对违规格式重生成最多 2 次，仍违规则告警放行（不丢档案）。

death Fact 语句格式：「{文明}的{人名}（{身份}）已于{年}因{死因}辞世，此后不得…」——带文明名前缀，让 LLM 能区分不同文明的同名者。

关键约定：
- **按 id 匹配，不按名字**：`Fact.subject` 与 `Artifact.author_id` 都是 `Person.id`。
  人物名字库小会撞名，按名字匹配会让同名活人替死人背锅。新增校验规则时务必用 id。
- **death Fact 标文明**：statement 开头带 `{civ}的` 前缀，配合 diary 跨文明同名澄清 prompt。
...
- **人物归并按全文明**：`_resolve_persons` 查人从单文明扩到全文明——LLM 写 A 文明文本时
  提及 B 文明人物是合理的，跨文明命中则归并原卡、不建同名副本。
...
**1. 命名规范（`models.NamingStyle`）** —— 结构化词库+模板+风格说明，每文明一套，
   可入 `config.yaml`、可演化。词库是**风格示例+种子**，非封闭选择集：`NameGenerator`
   的 prompt 把词库标为「示例调性，不限其中，鼓励发明新词」，LLM 真正按 style_note 即兴
   创造名字（而非在词库里随机挑——那是把 LLM 当随机数生成器用）。LLM 造的新词根/氏族由
   `_recycle_new_lexemes` 解析回收进 `civ.naming.roots`/`clans`（每类上限 30 防膨胀），词库
   随使用增长、撞名率递降。mock 兜底从现有词库随机组合（不回收，测试用）。演化由
   `engine._maybe_evolve_naming` 在科技升级（加意象词根）/政体更替（改 style_note）时触发，
   写 `Fact(kind="naming_reform")`。
...
**3. 非自然死亡（`engine._maybe_kill_notables` + `_kill_person`）**：填补「战争/瘟疫只削数值
   ...
   - **身份相关意外**：每 tick 按 `ACCIDENT_BASE(0.02) × SOCIAL_CLASS_BASE × ROLE_RISK` 判定。
...
**3. 人物关系图（`Person.relations` + `models.inverse_relation`）**：`relations` 是
   ...
   兜底：`naming.AppellationJudge` 判断称谓当名（秋子之父/阿爸/抄经人/老王）→弃用→生成真名，
   关系边照常建。LLM 主判（能识别规则覆盖不到的称谓，又不误杀真名阿兰/冉阿让），
   空返回或异常回退 `_looks_like_appellation` 规则。**关键**：`max_tokens` 给到 120+——
   DeepSeek 在 token 预算极小时返回空串，早期实现因 `max_tokens=10` 致空返回被当成真名
   （「全判真名」假象），故空返回须回退规则而非当真名。`_normalize_relation` 清洗关系字段
...
- **称谓当名用 LLM 判别+规则兜底**：`AppellationJudge` 主判，空返回回退 `_looks_like_appellation`；`max_tokens` 须 ≥120 防空返回。
- **新建平民走 register_commoner**：挂上 max_age/死因机制，参与自然死亡与意外死亡。
- **`civ_card` 不列全部人**：全员建档下人物多，只列在世名人 + 最近提及的 8 位平民，
  防上下文爆炸；完整卡按需检索。

## 社会组织（阶段1：实体 + 涌现 + 纵向 + 成员 + 约束式写作）

文明内部涌现可持久化的组织（村/教会/议会/行会/学堂...），让文明内部不再是扁平人物列表。

**1. 组织类型（`models.OrgType` 枚举）**：SETTLEMENT/CHURCH/GUILD/SCHOOL/COUNCIL/FIEF/ARMY/OTHER
   ——骨架，确定性逻辑用；具体组织名（如「霜鲸湾渔会」「海灵教会」）自由命名——与
   SocialClass/role 同模式。

**2. 组织实体（`models.Organization`）**：纵向隶属 `parent_org_id`；双向成员关系
   （`members`/`officers` + `Person.orgs`）；生命周期 `founded_year`/`dissolved_year`；
   经历条目 `history_entries`；`last_mentioned_year`（清理用）。

**3. 涌现（`naming.OrgProposer`）**：关键事件（科技升级/政体更替）时 `_maybe_propose_orgs`
   调 LLM 提议契合的新组织（如君主制→王廷+近卫军），建卡并写 `Fact(kind="org_emergence")`。
   mock 兜底 `_MOCK_ORG_CANDIDATES`。遵循判断/提议类调用健壮性约定。

**4. 约束式写作（替代事后抽取/校验）**：`civ_card` 注入存续组织清单；`world_brief` 加硬约束
   「文本只能引用上文已列出的存续组织，不得新造具体组织名（泛称场所如村/市集不限）；人物所属
   组织须与上文一致」。从源头堵，**不做组织 CAST 抽取块、不做组织校验**——约束在生成时。

**5. 新人物归组织**：CAST 加「所属组织名」列（第 11 列），引擎按名查该文明组织（校验存在
   才建成员关系）；未填或填不存在组织时，`OrgMemberInferrer` 按阶层/role/home 推断兜底
   （clergy→CHURCH、soldier→ARMY、artisan→GUILD、commoner/nobility→SETTLEMENT/FIEF/COUNCIL）；
   无合适则暂留空。`engine.assign_to_org` 建双向成员关系。

关键约定：
- **组织约束式引用**：LLM 只能引用既有组织，不新造——不靠事后校验。
- **新人物组织归入**：CAST 给线索（准）+ 校验存在性（不违反约束）+ 阶层推断兜底（不空）。
- **阶段1边界**：暂无解散触发/清理（接口留）、横向组织交流、组织命名风格、组织参与事件机械效果
  ——阶段后续。
